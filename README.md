...
ここでdpテーブルで更新が生じないことに注目すると、( (2,1) から Aを選ぶか、(2,2)からBを選ぶかで(3,2) に遷移するがどちらも同じ値になる) dp[y][x] は先にx冊だけAを読んでから、y-x 冊だけ Bを読む場合に生じる時間を示している。
Aを x冊読む場合 B を何冊まで読めるかは二分探索で簡単にわかる。(単調性があるため)

したがって、先に何冊Aを読むかで全探索し、Bを何冊読めるか二分探索で求めて、合計の最大値を求めればよいと考えた。

* 20221225 追記

dp できそう　→ きつい　→ A,B の単調性に注目し、Aを全探索して B をどこまで読めるか求めた ! って思考の流れなのはわかったけど、なんでこの流れになれたか不思議すぎる
...
# 217E  Sorting Queries  diff 986　(1226執筆)
...
データを管理する場所を 2か所作成し一つを優先度付きキュー(ヒープ)にする。ソートに対応する操作は、ヒープでない方に入っているデータを全てヒープに移すことで対応すれば、O(XlogX) の計算を削減できると考えた。

他の操作は、データの追加はヒープでない方のみで行い、出力はヒープにデータが存在する限りヒープから、なければヒープでない方の先頭から、行うことで対応できると考えた。

データを移す操作をヒープを作りなおしてしまったため　TLE することになったが、なんとか工夫して AC できた。

➁ 別解

より高速かつ綺麗に作成したので、12月ページで詳細を見ろ
...
辺で全探索し、その辺を使わない場合に最小全域木が構成できるか否か判定する。最小全域木の作成は O(M* logN * logM) なのでこれを辺の数だけ繰り返して全体として O(M^2 * logN * logM) となるが十分高速

* 最小全域木計算量は ソートでlogM , M 回のuf操作で MlogN で計 O(M* logN * logN)  
...
8-4 同様 O( M^2 logN logM ) では間に合わない制約。そこで、最小全域木に含まれる N-1 本のみに絞った判定にすることで　O( NM logN logM) ≓ O(10^7) に削減できる

* なぜか2000ms を超えてしまうがAC
